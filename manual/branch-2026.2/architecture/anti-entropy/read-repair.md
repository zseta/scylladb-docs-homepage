...
On read operations, ScyllaDB runs a process called [read repair](https://docs.scylladb.com/manual/branch-2026.2/reference/glossary.md#term-Read-Repair), to ensure that replicas are updated with most recently updated data. Such repairs during read path run automatically, asynchronously, and in the background.
...
A normal read operation works according to the scenarios described in our [Fault Tolerance documentation](https://docs.scylladb.com/manual/branch-2026.2/architecture/architecture-fault-tolerance.md).
...
3. If the digests from all replicas match, the coordinator responds to the client with the read data. If there is a mismatch (shown above), a [read repair](https://docs.scylladb.com/manual/branch-2026.2/reference/glossary.md#term-Read-Repair) is performed.
...
1. During [reconciliation](https://docs.scylladb.com/manual/branch-2026.2/reference/glossary.md#term-Reconciliation), the co-ordinator (Node *V*) compares full data sets, and **sends updates back** to the out-of-date nodes.
...
* [ScyllaDB Anti-Entropy](https://docs.scylladb.com/manual/branch-2026.2/architecture/anti-entropy/index.md)
