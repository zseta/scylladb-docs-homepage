...
The following procedure specifies how to add a Data Center (DC) to a live ScyllaDB Cluster, in a single data center, [multi-availability zone](https://docs.scylladb.com/manual/branch-2026.2/faq.md#faq-best-scenario-node-multi-availability-zone), or multi-datacenter. Adding a DC out-scales the cluster and provides higher availability (HA).
...
3. Install the new **clean** ScyllaDB nodes (See [Clean Data from Nodes]() below) on the new datacenter, see [Getting Started](https://docs.scylladb.com/manual/branch-2026.2/getting-started/index.md) for further instructions, create as many nodes that you need.
   Follow the ScyllaDB install procedure up to `scylla.yaml` configuration phase.
   In the case that the node starts during the installation process follow [these instructions](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/clear-data.md).
...
You can check the status of the nodes in the cluster using the [nodetool status](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/status.md) command.
...
#### WARNING
Make sure **all** your keyspaces are using `NetworkTopologyStrategy`. If this is not the case, follow the [these instructions](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/update-topology-strategy-from-simple-to-network.md) to fix that.
...
2. Set the DC and Rack Names.
   ...
   Ec2MultiRegionSnitch gives each DC and rack default names, the region name defined as datacenter name, and [availability zones](https://docs.scylladb.com/manual/branch-2026.2/faq.md#faq-best-scenario-node-multi-availability-zone) are defined as racks within a datacenter.
...
8. When all nodes are up and running `ALTER` the following Keyspaces in the new nodes:
   ...
   * `audit` - if enabled - replicate the data to three nodes in the new DC.
   ...
   #### NOTE
   If `rf_rack_valid_keyspaces` option is set, a tablet keyspace needs to use rack list replication factor, so that a new DC (rack) can be added. See [the conversion procedure](https://docs.scylladb.com/manual/branch-2026.2/cql/ddl.md#conversion-to-rack-list-rf). In this case, to add a datacenter:
   ...
   Consider [upgrading rf_rack_valid_keyspaces option to enforce_rack_list option](https://docs.scylladb.com/manual/branch-2026.2/architecture/tablets.md#keyspace-rf-rack-valid-to-enforce-rack-list) to ensure all tablet keyspaces use rack lists.
   ...
   You can abort the keyspace alteration using [Task manager](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/admin-tools/task-manager.md).
9. If any vnode keyspace was altered, run `nodetool rebuild` on each node in the new datacenter, specifying the existing datacenter name in the rebuild command.
...
10. If any vnode keyspace was altered, run a full cluster repair, using [nodetool repair -pr](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/repair.md) on each node, or using [ScyllaDB Manager ad-hoc repair](https://manager.docs.scylladb.com/stable/repair)
11. If you are using ScyllaDB Monitoring, update the [monitoring stack](https://monitoring.docs.scylladb.com/stable/install/monitoring_stack.html#configure-scylla-nodes-from-files) to monitor it. If you are using ScyllaDB Manager, make sure you install the [Manager Agent](https://manager.docs.scylladb.com/stable/install-scylla-manager-agent.html) and Manager can access the new DC.
