...
* Check the cluster status using the [nodetool status](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/status.md) command to understand the cluster deployment.
...
3. ALTER every cluster KEYSPACE, so that the keyspaces will no longer replicate data to the decommissioned data-center.
   ...
   #### NOTE
   If `rf_rack_valid_keyspaces` option is set, a tablet keyspace needs to use rack list replication factor, so that the DC can be removed. See [the conversion procedure](https://docs.scylladb.com/manual/branch-2026.2/cql/ddl.md#conversion-to-rack-list-rf). In this case, to remove a datacenter:
   ```shell
   cqlsh> DESCRIBE nba3
...
4. Run [nodetool decommission](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/decommission.md) on every node in the data center that is to be removed.
   Refer to [Remove a Node from a ScyllaDB Cluster - Down Scale](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/remove-node.md) for further information.

   For example:

   If ASIA-DC is to be removed, then, execute the `nodetool decommission` command on all the nodes in this data-center.
5. Verify that the data-center was successfully removed by using the [nodetool status](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/status.md) command.
