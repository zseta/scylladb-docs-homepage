...
1. Update the `authenticator` parameter in `scylla.yaml` for all the nodes in the cluster: Change `authenticator: AllowAllAuthenticator` to `authenticator: com.scylladb.auth.TransitionalAuthenticator`.
   ...
   ```
2. Run the [nodetool drain](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/drain.md) command (ScyllaDB stops listening to its connections from the client and other nodes).
...
4. Login over the maintenance socket and create an authenticated user with strong password.
   ...
   See [Setting Up a Superuser Using the Maintenance Socket](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/security/create-superuser.md#create-superuser-using-maintenance-socket) for instructions.
   ```cql
