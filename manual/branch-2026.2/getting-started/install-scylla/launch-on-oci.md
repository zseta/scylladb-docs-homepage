...
* A [VCN](https://docs.oracle.com/en-us/iaas/Content/Network/Tasks/managingVCNs.htm) with a subnet to
  launch the instance into, and the [ScyllaDB ports](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/security/security-checklist.md#networking-ports) allowed by the
  [security rules](https://docs.oracle.com/en-us/iaas/Content/Network/Concepts/securityrules.htm) that
  ...
  [ScyllaDB Security Checklist](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/security/security-checklist.md) for the full set of
  recommendations.
* An SSH key pair. See
  [Managing Key Pairs on Linux Instances](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/managingkeypairs.htm).
* To use the CLI: the [OCI CLI](https://docs.oracle.com/en-us/iaas/Content/API/SDKDocs/cliinstall.htm),
...
[terminated](https://docs.oracle.com/en-us/iaas/Content/Compute/Tasks/terminatinginstance.htm) or
...
<a id="launch-on-oci-console"></a>

## Launching ScyllaDB from the OCI Console

1. Choose a shape. See [Cloud Instance Recommendations for OCI](https://docs.scylladb.com/manual/branch-2026.2/getting-started/cloud-instance-recommendations.md#system-requirements-oci) for the list
   of recommended shapes.

   Other shapes will work, but with lesser performance. If you choose a shape other than the recommended
   ones, make sure to run the [scylla_setup](https://docs.scylladb.com/manual/branch-2026.2/getting-started/system-configuration.md#system-configuration-scripts) script.
2. Sign in to the [OCI Console](https://cloud.oracle.com/) and select the region you want to deploy in.
3. Open the navigation menu, select **Marketplace**, and then select **All Applications**.
4. Search for *ScyllaDB* and select the ScyllaDB listing for the version you want to deploy. Listings are
   named after the ScyllaDB release, for example *ScyllaDB 2026.2*.

   Alternatively, open
   [the ScyllaDB listings in Marketplace](https://cloud.oracle.com/marketplace/apps?searchQuery=ScyllaDB)
   directly.
...
7. Configure the instance:
   ...
   * **Advanced options > Management > Initialization script** – Optional. Provide the ScyllaDB
     configuration as user data. See [Configuring ScyllaDB with user data](#launch-on-oci-user-data)
     below.
...
8. Select **Create**.
   ...
   [Connecting to the instance](#launch-on-oci-connect).

<a id="launch-on-oci-cli"></a>
...
4. Get the package details of that version. The output holds the image OCID and the App Catalog
   ...
   ```
5. Retrieve the terms of use for that version. The response contains the EULA link, the Oracle Terms of Use
...
7. Launch the instance, using the image OCID from the package details:
   ...
   is optional; see [Configuring ScyllaDB with user data](#launch-on-oci-user-data). To list the
...
<a id="launch-on-oci-user-data"></a>
...
* `seed_provider` - The IP of the first node. New nodes will use the IP of this seed node to connect to
  the cluster and learn the cluster topology and state. See [ScyllaDB Seed Nodes](https://docs.scylladb.com/manual/branch-2026.2/kb/seed-nodes.md).
...
See [scylla.yaml](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/admin.md#admin-scylla-yaml) for information about the supported `scylla_yaml` options, and
the [ScyllaDB Image documentation](https://github.com/scylladb/scylla-machine-image) for the full
...
from within the VCN, through a
[bastion](https://docs.oracle.com/en-us/iaas/Content/Bastion/Concepts/bastionoverview.htm), or over
a VPN.
