...
|-----------|-------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 node    | Schema and topology updates are possible and safe.                                  | Try restarting the node. If the node is dead, [replace it with a new node](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                                         | Action to take                                                                                                                                                                                     |
|-----------|-------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-2 nodes | Schema and topology updates are possible and safe.                                  | Try restarting the node(s). If the node is dead, [replace it with a new node](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                                         | Action to take                                                                                                                                                                                                                                            |
|-----------|-------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-4 nodes | Schema and topology updates are possible and safe.                                  | Try restarting the nodes. If the nodes are dead, [replace them with new nodes](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node-or-more.md).                                               |
| 1 DC      | Schema and topology updates are possible and safe.                                  | When the DC comes back online, try restarting the nodes in the cluster. If the nodes are dead, [add 3 new nodes in a new region](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/add-dc-to-existing-dc.md). |
...
See [Verifying that consistent topology changes are enabled](https://docs.scylladb.com/manual/branch-2026.2/architecture/raft.md#verifying-consistent-topology-changes-enabled).
...
[node replacement procedure](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node.md)). Finally, you
...
* Ensure all live nodes are in the normal state using
  [nodetool status](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/status.md). If there is a node
  ...
  [nodetool status](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/nodetool-commands/status.md) on any other node.
  ...
  [node removal procedure](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/remove-node.md).
* Check if the cluster lost data. If the number of dead nodes is equal or larger than your
  ...
  [restore the data from backup](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/backup-restore/restore.md).
* Decide whether to shut down your service for the manual recovery procedure. ScyllaDB
  ...
  * restarting a single node could lead to unavailability of data queries (the procedure involves
    a [rolling restart](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/config-change/rolling-restart.md)). For
...
1. Perform a [rolling restart](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/config-change/rolling-restart.md) of your live nodes.
...
4. Perform the following queries on **every live node**:
   ...
   ```
5. Perform a [rolling restart](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/config-change/rolling-restart.md) of all live nodes,
   but:
...
6. Replace all dead nodes in the cluster using the
   [node replacement procedure](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node.md).

   #### NOTE
   Removing some of the dead nodes with the
   [node removal procedure](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/remove-node.md) is also possible,
