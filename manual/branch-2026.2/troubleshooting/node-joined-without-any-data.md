...
#### NOTE
This procedure only covers the situation in which a new node joins the cluster. Nodes replacing existing nodes following the [Replace A Dead Node](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/replace-dead-node.md) procedure are expected to be in `UN` state from the start.
...
The scenario constitutes a user error and is the incorrect way of adding new nodes to the cluster. See [Adding New Node](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/add-node-to-cluster.md) for more details. This article explains how to restore a cluster safely in such a case.
...
3. Once all the new nodes are decommissioned, add them again with the [correct procedure](https://docs.scylladb.com/manual/branch-2026.2/operating-scylla/procedures/cluster-management/add-node-to-cluster.md).
