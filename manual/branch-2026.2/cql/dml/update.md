...
(see [update ordering](https://docs.scylladb.com/manual/branch-2026.2/cql/dml.md#update-ordering)).
...
- `c = c + 3` is used to increment/decrement counters. The column name after the ‘=’ sign **must** be the same as
  ...
  operations allowed on counters. See the section on [counters](https://docs.scylladb.com/manual/branch-2026.2/cql/types.md#counters) for details.
- `id = id + <some-collection>` and `id[value1] = value2` are for collections, see the [relevant section](https://docs.scylladb.com/manual/branch-2026.2/cql/types.md#collections) for details.
