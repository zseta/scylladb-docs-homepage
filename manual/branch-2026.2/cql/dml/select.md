...
If your `SELECT` query results in what appears to be missing data, see this [KB Article](https://docs.scylladb.com/manual/branch-2026.2/kb/cqlsh-results.md) for information.
...
- A call to the [COUNT function](https://docs.scylladb.com/manual/branch-2026.2/cql/functions.md#count-function), which counts all non-null results.
...
You can read more about the `TIMESTAMP` retrieved by `WRITETIME` in the [UPDATE](https://docs.scylladb.com/manual/branch-2026.2/cql/dml.md#update-parameters) section.

- `TTL` retrieves the remaining time to live (in *seconds*) for the value of the column, if it set to expire, or `null` otherwise.

You can read more about TTL in the [documentation](https://docs.scylladb.com/manual/branch-2026.2/cql/time-to-live.md) and also in [this ScyllaDB University lesson](https://university.scylladb.com/courses/data-modeling/lessons/advanced-data-modeling/topic/expiring-data-with-ttl-time-to-live/).
...
option. See [CREATE TABLE](https://docs.scylladb.com/manual/branch-2026.2/cql/ddl.md#create-table-statement).
...
Currently, the possible orderings are limited by the [clustering order](https://docs.scylladb.com/manual/branch-2026.2/cql/ddl.md#clustering-order) defined on the table:
...
`vector` is the query [vector](https://docs.scylladb.com/manual/branch-2026.2/cql/types.md#vectors), and `LIMIT` is a limit on the number of results to return.
Vector queries can only be performed on a vector column that has a [vector index](https://docs.scylladb.com/manual/branch-2026.2/cql/secondary-indexes.md#create-vector-index-statement) created.
...
[creating the vector index](https://docs.scylladb.com/manual/branch-2026.2/cql/secondary-indexes.md#create-vector-index-statement).
...
There’s also possibility to return the similarity score along with the results by using the [similarity functions](https://docs.scylladb.com/manual/branch-2026.2/cql/functions.md#vector-similarity-functions).
