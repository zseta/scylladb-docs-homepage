...
ScyllaDB replicates data according to a [replication](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Replication) strategy that you choose. This strategy will determine the placement of the replicated data.  ScyllaDB runs nodes in a hash ring. All nodes are equal: there are no master, slave, or replica sets.

The [Replication Factor (RF)](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Replication-Factor-RF) is equivalent to the number of nodes where data (rows and partitions) are replicated.   Data is replicated to multiple (RF=N) nodes.
...
During a [read operation](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Read-Operation), the client sends a request to the coordinator.   Effectively because the RF=3, 3 nodes respond to the read request.

The [Consistency Level (CL)](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Consistency-Level-CL) determines how many replicas in a cluster must acknowledge read or [write operations](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Write-Operation) before it is considered successful.

For the CQL Shell ([CQLsh](https://docs.scylladb.com/manual/branch-2026.1/cql/cqlsh.md)), the consistency level defaults to ONE for read and write operations.
...
The Consistency Level is tunable per operation in CQL.   This is known as [tunable consistency](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-Tunable-Consistency). Sometimes response latency is more important, making it necessary to adjust settings on a per-query or operation level to override keyspace or even data center-wide consistency settings.  In other words, the Consistency Level setting allows you to choose a point in the consistency vs. latency tradeoff.
...
Refer to the [Consistency Level table](https://docs.scylladb.com/manual/branch-2026.1/cql/consistency.md#consistency-levels-reference) to get information about the Consistency Levels that are available for a read or write operation.

ScyllaDB, as do many distributed database systems, adheres to the [CAP Theorem](https://docs.scylladb.com/manual/branch-2026.1/reference/glossary.md#term-CAP-Theorem).  The **CAP Theorem** is the notion that **Consistency**, **Availability** and **Partition Tolerance** of data are mutually dependent in a distributed system. Increasing any 2 of these factors will reduce the third.
...
* [Consistency Level Console Demo](https://docs.scylladb.com/manual/branch-2026.1/architecture/console-CL-full-demo.md)
* [Consistency Levels](https://docs.scylladb.com/manual/branch-2026.1/cql/consistency.md)
