# Upgrade from ScyllaDB 2025.x to ScyllaDB 2026.1

This document describes a step-by-step procedure for upgrading from ScyllaDB 2025.x
to ScyllaDB 2026.1 and rollback to version 2025.x if necessary.
...
See [About Upgrade](https://docs.scylladb.com/manual/branch-2026.1/upgrade/about-upgrade.md) for the ScyllaDB upgrade policy.
...
* Not to use the new 2026.1 features.
...
1. Update the ScyllaDB deb repo to 2026.1.
   > ```console
   > sudo wget -O /etc/apt/sources.list.d/scylla.list https://downloads.scylladb.com/deb/debian/scylla-2026.1.list
   > ```
...
1. Update the ScyllaDB rpm repo to 2026.1.
   > ```console
   > sudo curl -o /etc/yum.repos.d/scylla.repo -L https://downloads.scylladb.com/rpm/centos/scylla-2026.1.repo
   > ```
2. Install the new ScyllaDB version:
   > ```sh
   > sudo yum clean all
...
upgraded to 2026.1 yet. As soon as the last node in the rolling upgrade
procedure is started with 2026.1, rollback becomes impossible. At that
point, the only way to restore a cluster to 2025.x is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 2026.1 to
2025.x. Apply this procedure if an upgrade from 2025.x to
2026.1 fails before completing on all nodes.

* Use this procedure only on the nodes you upgraded to 2026.1.
...
For each of the nodes you rollback to 2025.x, serially (i.e., one node
at a time), you will:
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 2025.x packages backed up during the upgrade.
   > ```sh
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 2025.x packages backed up during the upgrade procedure.
   > ```sh
...
1. Restore the 2025.x packages backed up during the upgrade
