...
1. Set `internode_compression_enable_advanced` to `true`.
2. Set `internode_compression_zstd_max_cpu_fraction` to
...
1. Set `internode_compression_enable_advanced` to `true`.
2. Configure the `rpc_dict_training_when` parameter.
   In typical scenarios, you should set it to `when_leader`.
   As a result, the [Raft leader](https://docs.scylladb.com/manual/branch-2026.1/architecture/raft.md) node will
...
See the *Inter-node settings* section in [Configuration Parameters](https://docs.scylladb.com/manual/branch-2026.1/reference/configuration-parameters.md)
