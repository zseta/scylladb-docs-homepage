...
* Ensure that all the relevant [ports](https://docs.scylladb.com/manual/branch-2026.1/operating-scylla/admin.md#cqlsh-networking) are open in your EC2 Security Group.
...
1. Install ScyllaDB on the nodes you want to add to the cluster. See [Getting Started](https://docs.scylladb.com/manual/branch-2026.1/getting-started/index.md) for installation instructions and
   follow the procedure up to  the `scylla.yaml` configuration phase.

   If the ScyllaDB service is already running (for example, if you are using [ScyllaDB AMI](https://www.scylladb.com/download/?platform=aws)), stop it before moving to the next step by using [these instructions](https://docs.scylladb.com/manual/branch-2026.1/operating-scylla/procedures/cluster-management/clear-data.md).
2. On each node, edit the `scylla.yaml` file in `/etc/scylla/` to configure the parameters listed below. See the [EC2 Configuration Table](#table) above on how to configure your cluster.
   ...
   > * **seeds** - Specify the IP of the node you chose to be a seed node. See [ScyllaDB Seed Nodes](https://docs.scylladb.com/manual/branch-2026.1/kb/seed-nodes.md) for details.
