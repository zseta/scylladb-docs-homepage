# Upgrade from ScyllaDB 2025.x to ScyllaDB 2025.4

This document describes a step-by-step procedure for upgrading from ScyllaDB 2025.x
to ScyllaDB 2025.4 and rollback to version 2025.x if necessary.

This guide covers upgrading ScyllaDB on Red Hat Enterprise Linux (RHEL), CentOS, Debian,
and Ubuntu. See [OS Support by Platform and Version](https://docs.scylladb.com/manual/branch-2025.4/getting-started/os-support.md)
...
* Not to use the new 2025.4 features.
...
1. Update the ScyllaDB deb repo to 2025.4.
   > ```console
...
1. Update the ScyllaDB rpm repo to 2025.4.
   > ```console
...
upgraded to 2025.4 yet. As soon as the last node in the rolling upgrade
procedure is started with 2025.4, rollback becomes impossible. At that
point, the only way to restore a cluster to 2025.x is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 2025.4 to
2025.x. Apply this procedure if an upgrade from 2025.x to
2025.4 fails before completing on all nodes.

* Use this procedure only on the nodes you upgraded to 2025.4.
