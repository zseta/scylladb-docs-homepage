...
* in a vnode-based keyspace, the currently operating *CDC generation* ([CDC Stream Changes](https://docs.scylladb.com/manual/branch-2025.4/features/cdc/cdc-stream-changes.md)),
...
#### NOTE
To make the above example, we simply kept inserting rows with different partition keys until we found two rows that went to the same stream.
