...
* Check the cluster status using the [nodetool status](https://docs.scylladb.com/manual/branch-2025.4/operating-scylla/nodetool-commands/status.md) command to understand the cluster deployment.
...
3. ALTER every cluster KEYSPACE, so that the keyspaces will no longer replicate data to the decommissioned data-center.
   ...
   #### NOTE
   If `rf_rack_valid_keyspaces` option is set, the DC cannot be removed. To proceed, first restart all nodes in the cluster disabling this option, then follow the procedure described above. If `rf_rack_valid_keyspaces` is disabled, no [Materialized Views](https://docs.scylladb.com/manual/branch-2025.4/cql/mv.md) are allowed.
4. Run [nodetool decommission](https://docs.scylladb.com/manual/branch-2025.4/operating-scylla/nodetool-commands/decommission.md) on every node in the data center that is to be removed.
   Refer to [Remove a Node from a ScyllaDB Cluster - Down Scale](https://docs.scylladb.com/manual/branch-2025.4/operating-scylla/procedures/cluster-management/remove-node.md) for further information.

   For example:

   If ASIA-DC is to be removed, then, execute the `nodetool decommission` command on all the nodes in this data-center.
5. Verify that the data-center was successfully removed by using the [nodetool status](https://docs.scylladb.com/manual/branch-2025.4/operating-scylla/nodetool-commands/status.md) command.
