...
#### NOTE
Use [this article](https://docs.scylladb.com/manual/branch-2025.4/troubleshooting/large-partition-table.md) to help you find large partitions.
...
1. Set  the `gc_grace_seconds` to the time since last repair was started -  For instance, if the last repair was executed one day ago, then set `gc_grace_seconds` to one day (86400sec). For more information, please refer to [this KB article](https://docs.scylladb.com/manual/branch-2025.4/kb/gc-grace-seconds.md).
...
#### NOTE
By default, major compaction runs on all the keyspaces and tables, so if we want to specyfy e.g. only one table, we should point at it using arguments: `<keyspace>.<mytable>`. For more information, please see [Nodetool compact](https://docs.scylladb.com/manual/branch-2025.4/operating-scylla/nodetool-commands/compact.md).
