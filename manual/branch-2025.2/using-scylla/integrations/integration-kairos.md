...
1. Configure KairosDB to connect to the ScyllaDB server.
   ...
   * In the  `#Cassandra properties` section, set the ScyllaDB nodes IP
     ...
     ```
   * Set the [replication](https://docs.scylladb.com/manual/branch-2025.2/architecture/architecture-fault-tolerance.md) factor (for production purposes use a ScyllaDB cluster with a minimum of RF=3)
     ```none
     kairosdb.datastore.cassandra.replication={'class': 'NetworkTopologyStrategy','replication_factor' : 3}
     ```
   * Set the read and write [consistency](https://docs.scylladb.com/manual/branch-2025.2/architecture/architecture-fault-tolerance.md) level (for production purposes use write=ONE, read=QUORUM)
     ```none
