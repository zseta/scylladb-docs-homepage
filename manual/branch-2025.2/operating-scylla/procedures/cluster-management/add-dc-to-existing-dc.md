...
The following procedure specifies how to add a Data Center (DC) to a live ScyllaDB Cluster, in a single data center, [multi-availability zone](https://docs.scylladb.com/manual/branch-2025.2/faq.md#faq-best-scenario-node-multi-availability-zone), or multi-datacenter. Adding a DC out-scales the cluster and provides higher availability (HA).
...
3. Install the new **clean** ScyllaDB nodes (See [Clean Data from Nodes]() below) on the new datacenter, see [Getting Started](https://docs.scylladb.com/manual/branch-2025.2/getting-started/index.md) for further instructions, create as many nodes that you need.
   Follow the ScyllaDB install procedure up to `scylla.yaml` configuration phase.
   In the case that the node starts during the installation process follow [these instructions](https://docs.scylladb.com/manual/branch-2025.2/operating-scylla/procedures/cluster-management/clear-data.md).
...
You can check the status of the nodes in the cluster using the [nodetool status](https://docs.scylladb.com/manual/branch-2025.2/operating-scylla/nodetool-commands/status.md) command.
...
#### WARNING
Make sure **all** your keyspaces are using `NetworkTopologyStrategy`. If this is not the case, follow the [these instructions](https://docs.scylladb.com/manual/branch-2025.2/operating-scylla/procedures/cluster-management/update-topology-strategy-from-simple-to-network.md) to fix that.
...
2. Set the DC and Rack Names.
   ...
   Ec2MultiRegionSnitch gives each DC and rack default names, the region name defined as datacenter name, and [availability zones](https://docs.scylladb.com/manual/branch-2025.2/faq.md#faq-best-scenario-node-multi-availability-zone) are defined as racks within a datacenter.
...
3. Run a full cluster repair, using [nodetool repair -pr](https://docs.scylladb.com/manual/branch-2025.2/operating-scylla/nodetool-commands/repair.md) on each node, or using [ScyllaDB Manager ad-hoc repair](https://manager.docs.scylladb.com/stable/repair)
