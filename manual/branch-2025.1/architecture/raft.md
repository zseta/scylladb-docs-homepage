...
strong data consistency ([LWT](https://docs.scylladb.com/manual/branch-2025.1/features/lwt.md)). To achieve stronger consistency without performance penalty, ScyllaDB has turned to Raft - a consensus algorithm designed as an alternative to both gossip and Paxos.
...
See [Configuration Parameters](https://docs.scylladb.com/manual/branch-2025.1/reference/configuration-parameters.md) for details about the `join_ring` option.

<a id="raft-schema-changes"></a>

## Safe Schema Changes with Raft

In ScyllaDB, schema is based on [Data Definition Language (DDL)](https://docs.scylladb.com/manual/branch-2025.1/cql/ddl.md). In earlier ScyllaDB versions, schema changes were tracked via the gossip protocol, which might lead to schema conflicts if the updates are happening concurrently.
...
the [manual recovery procedure](https://docs.scylladb.com/manual/branch-2025.1/troubleshooting/handling-node-failures.md#recovery-procedure).
...
* If all nodes are alive and the network is healthy, perform a [rolling restart](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/config-change/rolling-restart.md) of the cluster.
...
If some nodes are **dead and irrecoverable**, you’ll need to perform a manual recovery procedure. Consult [the section about Raft recovery](https://docs.scylladb.com/manual/branch-2025.1/troubleshooting/handling-node-failures.md#recovery-procedure).
...
If `value` is `recovery`, it means that the cluster is in the middle of the manual recovery procedure. The procedure must be finished. Consult [the section about Raft recovery](https://docs.scylladb.com/manual/branch-2025.1/troubleshooting/handling-node-failures.md#recovery-procedure).
...
See [Handling Node Failures](https://docs.scylladb.com/manual/branch-2025.1/troubleshooting/handling-node-failures.md).
