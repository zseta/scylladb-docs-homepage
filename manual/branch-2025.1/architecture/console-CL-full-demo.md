...
In this demo, we’ll bring up 3 nodes and demonstrate how writes and reads look, with tracing enabled in a cluster where our [Replication Factor (RF)](https://docs.scylladb.com/manual/branch-2025.1/reference/glossary.md#term-Replication-Factor-RF) is set to **3**.  We’ll change the [Consistency Level (CL)](https://docs.scylladb.com/manual/branch-2025.1/reference/glossary.md#term-Consistency-Level-CL) between operations to show how messages are passed between nodes, and finally take down a few nodes to show failure conditions in a ScyllaDB cluster.
...
First, we’ll bring up a 3-node docker cluster.  You’ll see all the nodes eventually read  `UN` for status. `U` means **up**, and `N` means **normal**.  Read more about [Nodetool Status](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/nodetool-commands/status.md).
