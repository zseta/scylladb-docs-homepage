...
|-----------|-------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 node    | Schema and topology updates are possible and safe.                                  | Try restarting the node. If the node is dead, [replace it with a new node](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                                         | Action to take                                                                                                                                                                                     |
|-----------|-------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-2 nodes | Schema and topology updates are possible and safe.                                  | Try restarting the node(s). If the node is dead, [replace it with a new node](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                                         | Action to take                                                                                                                                                                                                                                            |
|-----------|-------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-4 nodes | Schema and topology updates are possible and safe.                                  | Try restarting the nodes. If the nodes are dead, [replace them with new nodes](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/replace-dead-node-or-more.md).                                               |
| 1 DC      | Schema and topology updates are possible and safe.                                  | When the DC comes back online, try restarting the nodes in the cluster. If the nodes are dead, [add 3 new nodes in a new region](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/add-dc-to-existing-dc.md). |
...
* [The Raft upgrade procedure](https://docs.scylladb.com/manual/branch-2025.1/architecture/raft.md#verify-raft-procedure)
...
#### WARNING
The manual recovery procedure is not supported [if tablets are enabled on any of your keyspaces](https://docs.scylladb.com/manual/branch-2025.1/architecture/tablets.md).
In such a case, you need to [restore from backup](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/backup-restore/restore.md).

During the manual recovery procedure you’ll enter a special `RECOVERY` mode, remove
all faulty nodes (using the standard [node removal procedure](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/remove-node.md)),
...
1. Perform the following query on **every alive node** in the cluster, using e.g. `cqlsh`:
   ...
   ```
2. Perform a [rolling restart](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/config-change/rolling-restart.md) of your alive nodes.
3. Verify that all the nodes have entered `RECOVERY` mode when restarting; look for one of the following messages in their logs:
   ...
   > ```
4. Remove all your dead nodes using the [node removal procedure](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/cluster-management/remove-node.md).
5. Remove existing Raft cluster data by performing the following queries on **every alive node** in the cluster, using e.g. `cqlsh`:
   ...
   ```
6. Make sure that schema is synchronized in the cluster by executing [nodetool describecluster](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/nodetool-commands/describecluster.md) on each node and verifying that the schema version is the same on all nodes.
7. We can now leave `RECOVERY` mode. On **every alive node**, perform the following query:
   ...
   ```
8. Perform a [rolling restart](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/config-change/rolling-restart.md) of your alive nodes.
9. The Raft upgrade procedure will start anew. [Verify](https://docs.scylladb.com/manual/branch-2025.1/architecture/raft.md#verify-raft-procedure) that it finishes successfully.
