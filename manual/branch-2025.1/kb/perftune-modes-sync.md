...
the `sq_split` mode instead of the `mq` mode (see [Seastar Perftune](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/admin-tools/perftune.md) for information about the modes).
...
4. Restart the `scylla-server` service.
   ...
   ```
5. Wait for the service to become up and running (similarly to how it is done during a [rolling restart](https://docs.scylladb.com/manual/branch-2025.1/operating-scylla/procedures/config-change/rolling-restart.md)). It may take a considerable amount of time before the node is in the UN state due to resharding.
