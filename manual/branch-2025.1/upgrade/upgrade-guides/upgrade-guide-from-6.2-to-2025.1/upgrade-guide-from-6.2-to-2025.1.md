# Upgrade from ScyllaDB Open Source 6.2 to ScyllaDB 2025.1

This document describes a step-by-step procedure for upgrading from ScyllaDB 6.2
to ScyllaDB 2025.1 and rollback to version 6.2 if necessary.
...
upgraded to 2025.1 yet. As soon as the last node in the rolling upgrade
procedure is started with 2025.1, rollback becomes impossible. At that
point, the only way to restore a cluster to 6.2 is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 2025.1.x to
6.2.y. Apply this procedure if an upgrade from 6.2 to
2025.1 fails before completing on all nodes.
...
For each of the nodes you rollback to 6.2, serially (i.e., one node
at a time), you will:
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 6.2 packages backed up during the upgrade.
   > ```sh
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 6.2 packages backed up during the upgrade procedure.
   > ```sh
...
1. Restore the 6.2 packages backed up during the upgrade
