# Upgrade from ScyllaDB 2025.2 to ScyllaDB 2025.3

This document describes a step-by-step procedure for upgrading from ScyllaDB 2025.2
to ScyllaDB 2025.3 and rollback to version 2025.2 if necessary.

This guide covers upgrading ScyllaDB on Red Hat Enterprise Linux (RHEL), CentOS, Debian,
and Ubuntu. See [OS Support by Platform and Version](https://docs.scylladb.com/manual/branch-2025.3/getting-started/os-support.md)
...
If you’re using a [ScyllaDB driver](https://docs.scylladb.com/manual/branch-2025.3/using-scylla/drivers/cql-drivers/index.md),
...
* Not to use the new 2025.3 features.
...
1. Update the ScyllaDB deb repo to 2025.3.
   > ```console
...
1. Update the ScyllaDB rpm repo to 2025.3.
   > ```console
...
upgraded to 2025.3 yet. As soon as the last node in the rolling upgrade
procedure is started with 2025.3, rollback becomes impossible. At that
point, the only way to restore a cluster to 2025.2 is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 2025.3.x to
2025.2.y. Apply this procedure if an upgrade from 2025.2 to
2025.3 fails before completing on all nodes.

* Use this procedure only on the nodes you upgraded to 2025.3.
...
For each of the nodes you rollback to 2025.2, serially (i.e., one node
at a time), you will:
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 2025.2 packages backed up during the upgrade.
   > ```sh
...
1. Remove the old repo file.
   ...
   > ```
2. Restore the 2025.2 packages backed up during the upgrade procedure.
   > ```sh
...
1. Restore the 2025.2 packages backed up during the upgrade
