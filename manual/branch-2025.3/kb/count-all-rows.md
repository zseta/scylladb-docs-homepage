...
One way to prevent that issue in ScyllaDB 4.4 or later is to increase the timeout for the query using the [USING TIMEOUT](https://docs.scylladb.com/manual/branch-2025.3/cql/dml/select.md#using-timeout) directive, for example:

```cql
SELECT COUNT(1) FROM ks.table USING TIMEOUT 120s;
```

You can also get an *estimation* of the number **of partitions** (not rows) with [nodetool tablestats](https://docs.scylladb.com/manual/branch-2025.3/operating-scylla/nodetool-commands/tablestats.md).
